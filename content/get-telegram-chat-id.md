...
Group and supergroup IDs are often **negative numbers**. That is normal. When a group is upgraded to a supergroup, the chat_id can change. See [group vs supergroup chat_id](/telegram-group-chat-id-vs-supergroup/) for details.
...
See [bot not responding in a Telegram group](/bot-not-responding-in-group/) for a full checklist.

## Related pages

- [Add a bot to a group to get chat_id (10 seconds)](/add-bot-to-group-chat-id/)
...
Group and supergroup IDs are typically negative in the Telegram Bot API. This is by design. See [Telegram chat_id format](/telegram-chat-id-format/).
...
Yes. Both groups and supergroups are supported. The bot shows the group name and chat_id. See [group vs supergroup chat_id](/telegram-group-chat-id-vs-supergroup/).
